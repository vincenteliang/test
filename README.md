...
摘  要
在互联网的时代，互联网信息技术在各个领域都得到了深化的应用。对人们的生活以及学习产生了较大的影响。通过信息技术建立的校园迎新管理系统利用系统对校园迎新管理进行统一的管理，能够提升校园迎新管理的工作效率，具有重要的现实意义。
...
Abstract
In the era of Internet, Internet information technology has been deeply applied in various fields. It has a great influence on people's life and study. Through the information technology to establish the campus orientation management system, using the system to manage the campus orientation management, can improve the efficiency of the campus orientation management, has important practical significance.
...
1 绪论
1.1 课题研究背景与意义
自主化迎新管理系统是整个数字化校园建设的重要组成部分。在系统构建过程中规范化高校的迎新流程，标准化数据存储及数据统计是最根本的目标之一。如何提各高校在迎新工作的工作效率，如何通过计算机软件技术，通过一体化的线上流程报到，协助及引导学生通过学费确认、宿舍选取、军训服装订购及预约接站来替代复杂繁琐的传统报到方式，将成为系统建设的核心目标和意义。而后高校通过扫描枪，可以方便地从数据库里找出这个学生的相关信息，帮助他完成每一个环节的报到，真正实现数字化、网络化的一站式报到系统。
高校迎新系统常具有研究意义和存在价值，在各大高校的数字化建设中起到承上启下的重要作用。提供给高校进行合理安排学生宿舍、统一购置军训服装、安排每日接站班车、合理利用现有资源，充分发挥计算机的统计优势。

1.2 国内外研究现状
在整个教学过程中，除了入学系统以外，随着个人计算机的普及，还为其他多方面的教学提供了便利，如CAI的研究。比如80年代早期，美国的依利诺斯大学(University of Illinois at Urbana-Champaign)就已经开始了计算机辅助教学活动，也是当时比较先进的学校之一，建立了非常庞大的CAI中心。其计算机技术应用到教学中的各个部门，其中包括学生教学、科学研究、行政应用、入学，通过计算的方式替代了传统教学过程中的黑板加粉笔的方式。但由于环境及管理的诸多影响并没有完全普及，但为后续的数字化校园建设打开了先河。
...
4.3 系统网络拓扑设计
根据图4.2所示，校园迎新管理系统结构中客户端服务器、WEB服务器互联网的支持下从搜索库服务器提取有效的信息，从而使整个校园迎新管理系统仅有有效运作，为用户提供校园迎新管理服务。
 
图4.2  系统结构图
4.4 系统功能模块设计
系统设计应该以客户需求为主要实施原则，注重满足不同客户的差异性需求，以及大多数用户对于使用习惯上的共同体验，再根据这一原则做好相关技术设置上的程序编排。在具体操作中，首先应该成立数据库，设计库作为最基础的运行项目，决定其功能的建立和开展，其次在依据具体的需求进行架构选择和设计，选择一个合理的设计架构关系着系统的使用寿命和开发难度。系统功能模块如图4.3 所示。

 
图4.3系统功能模块图
1．缴费信息
学生通过录取通知书上的地址，账号及密码进登录到高校迎新管理系统后，系统首先判断此学生是否已经完成报到，如果未完成报到首先呈现的就是学校代缴费信息。缴费功能结构如图4.4所示。
 
图4.4 缴费功能结构
2.宿舍选取
当学生进入到宿舍选取模块时，系统会自动计算当前宿舍的占用情况，罗列出可选宿舍的信息，方便学生选取中意的宿舍，宿舍选取功能结构如图4.5所示。
 
图4.5 宿舍选取功能模块图
3．军训服装选择
军训服装选择比较简单，根据学生自己的衣服尺码、鞋子尺码进行尺码选择，军训服装选择功能结构如图4.6所示。
 
图4.6 军训服装选择功能结构
4．预约接站
预计接站服务是各大高校保证学生安全、准确的抵达学校的统一组织形式，但由于学生到校时间不同，通过此模块可以正确引导学生统一到达时间范围及陪同人信息，预计接站功能模块结构如图4.7所示。
 
图4.7 预计接站功能模块结构
4.5 系统数据库设计
4.5.1 E-R图设计
校园迎新管理系统中所有数据的存储都是以各个实体为中心，将各实体之间的关系关联起来，即形成E-R图。系统E-R图设计如图4.8所示。
...
图5.1 系统登录界面
用户登录成功之后进入系统的主页面，如图5.2所示。

图5.2 系统主页面
5.3.2 接站预约模块实现
接站预约是系统中独具特色及人性化的功能模块，使学生减少了学校报到的担忧，增强了学子外地求学的信心。接站预约功能如图5.3所示。

图5.3 预约接站界面
5.3.3 寝室选择模块实现
寝室选择是迎新系统中的必填项，根据学生选择的公寓不同，其收费价格也不同，系统需要根据实时数据计算出可选的公寓，宿舍及尚未被选择的床位，已便后续的新生选择。寝室选择如图5.4所示。
